...
void read_frame_buffer(uint8_t *buffer)
{
    ...
    printf("frame %d, size %d, divided into %d batches \n", frame_num, len, info.batch_num_total);
...
void mqtt_send_batch(void *arg)
{
    ...
    enum
    {
        JSON_BUF_SIZE = BASE64_BUF_SIZE + 200,
    };
    ...
    while (1)
    {
        ...
        cJSON_AddNumberToObject(root,"frame",info.label);
        cJSON_AddNumberToObject(root, "batch", info.batch_no);
        cJSON_AddStringToObject(root, "binary", (char *)base64_buf);
        cJSON_PrintPreallocated(root, json_buf, JSON_BUF_SIZE, false);
        mqtt_send(client, topic, json_buf, 2);
        cJSON_Delete(root);
...
extern "C" void app_main(void)
{
    ...
    myCAM.InitCAM();
    myCAM.set_JPEG_size(OV2640_1280x1024);
    myCAM.clear_fifo_flag();
